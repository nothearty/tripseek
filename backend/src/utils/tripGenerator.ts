  ...
  try {
    ...
    const request = `
      Generate a detailed trip itinerary in JSON format for the following prompt: 
      {
        ...
        "preferences": "${prompt.activities} ${prompt.other}"
    }
      ...
      Don't mention arrival / departure part. Don't repeat places you have already mentioned before other days! Ensure that the JSON follows this format exactly and that each field is filled out with realistic data. Use single quotes for strings, and ensure the JSON is valid, complete and closed correctly. Add extra } at the end.
    `
